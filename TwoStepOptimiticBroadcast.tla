...
(**************************************************************************************)
Message ==
    [src : P, dst : P, type : {"proposal", "echo", "vote", "ack", "ready"}, val : V]
...
l0:     while (TRUE) with (v \in V) {
            ...
            or { \* send ready
                ...
                await
                    \/  Cardinality({m \in Acks(self, v) : m.src # broadcaster}) >= CeilDiv(N+F-1, 2)
...
l1:     while (TRUE)
        with (v \in V)
        with (t \in {"proposal", "echo", "vote", "ack", "ready"})
        with (d \in P \ Faulty) {
