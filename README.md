...
You will then find a typeset version of the theories in `./OptiRBC/output/document.pdf`.
...
```bash
isabelle build -P /tmp/optirbc_browser -D OptiRBC
...
`-P` writes the presentation tree (`<group>/<session>/`) to the given directory; since `ROOT` does not set a group, the session lands under `Unsorted/OptiRBC/`. The `cp` line picks up both the HTML pages and `document.pdf`. Commit the refreshed files in `OptiRBC/browser_info/` so the links keep working.
